...
* a **head set** to the component substrate (``ITI_HEADS``): train a linear probe
  ...
  held-out probe AUROC, take the top-K (matched to the retrieval-head count for a
  fair Jaccard);
...
def select_iti_heads(
    ...
    *,
    ...
    a ``[n_layers, n_heads]`` :class:`HeadScore` of held-out AUROC. ``top_k`` is
    matched to the retrieval-head count for a fair Jaccard.
    """
