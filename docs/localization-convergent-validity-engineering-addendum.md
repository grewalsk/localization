...
Token-swap plays one **secondary** role now that masking is the primary oracle (§8.1): the **oracle-ranking robustness check** — re-derive the ranking under symmetric token replacement and report stability against the masking ranking (gate 11.2b), so a reviewer cannot claim the oracle is arbitrary. (The attribution-patching estimate, §8.3, linearizes masking directly and needs **no** corrupted reference — M9; the earlier mean-ablation form is retired.) The corruption must be chosen deliberately. Zhang and Nanda (arXiv 2309.16042) show Gaussian-noise corruption (the ROME style) is fragile and hyperparameter-sensitive. **Default: symmetric token replacement** (swap the gold span, or the needle, for a different plausible span so the answer is no longer supported), which is more stable and semantically controlled than noising embeddings, and stays on-manifold. Document-swap (replace the supporting document with a distractor) is a coarser alternative for the multi-hop sets. Do not use Gaussian noise. Note the swap variant changes neighboring positions' residual streams through their attention to the swapped token (it propagates from the embedding upward), which masking does not — one more reason masking, not swap, is the primary oracle.

### 8.3 Cheap approximation: attribution patching, validated against masking

Attribution patching (Nanda 2022; AtP*, Kramár et al. 2024) approximates an intervention's effect with one forward and one backward pass via a first-order expansion. To estimate the **masking** oracle (§8.1) we linearize the masking intervention itself — the **attention-pattern** form. Masking key `t` zeroes token `t`'s column in every post-softmax attention pattern `A[l,h,q,t]` (then renormalizes the survivors); to first order, with answer metric `m = logP(answer)`:

```
E_hat(t) ~= sum_{l,h,q}  A_clean[l,h,q,t]  *  grad_{A[l,h,q,t]} [ m ]
```

`A_clean` and `m` come from one forward pass and `grad m` from one backward pass — **no corrupted reference**, since the masking delta on column `t` is the analytic `-A_clean[...,t]`. This collapses the per-token masking sweep into a single backward pass, independent of the number of tokens. The sign matches `E(t) = m(x) - m(x^\t)`: a large positive `E_hat(t)` predicts masking `t` drops the answer log-prob. The renormalization of the surviving keys is the second-order term AtP* (Kramár et al. 2024) corrects; gate 11.2a certifies whether the bare first-order estimate suffices.
...
4. **Corruption type** for the robustness check. Section 8.2. The primary oracle is masking (§8.1) and the AP estimate linearizes masking directly (§8.3, M9); token-swap is the robustness-check corruption (gate 11.2b, default), never Gaussian noise.
