...
- **Replication model.** Gemma-2-9B-it with Gemma Scope 2, to show the disagreement structure is not model-specific. Qwen-Scope gives a third option for the cross-model extension.
...
- **Datasets.** Synthetic needle-in-a-haystack for controllable gold spans and needle-depth confounds; HotpotQA for multi-hop with supporting-fact annotations as gold spans; LongBench subsets for realistic long-context distribution. A single-hop set (Natural Questions or TriviaQA) gives the easy-versus-hard contrast.
