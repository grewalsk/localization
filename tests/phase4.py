...
Re-run gates 11.0 through 11.3 on ``gemma-2-9b-it`` with Gemma Scope 2. The
disagreement structure and the ``D(x)``-predicts-flips result either replicate or
they do not; either outcome is reportable. The whole gate is model-bound and
marked ``gpu``; the only thing checkable in CI is that the replication backbone is
pinned to the right model so the H100 run targets Gemma, not the primary.
"""
...
def test_11_4_phases_replicate_on_gemma(gemma_backbone):
    pytest.skip(
        "re-run gates 11.0-11.3 on gemma-2-9b-it with Gemma Scope 2 (§11.4); the "
        "disagreement structure and D(x)-predicts-flips result either replicate or "
        "they do not -- both are reportable"
    )
